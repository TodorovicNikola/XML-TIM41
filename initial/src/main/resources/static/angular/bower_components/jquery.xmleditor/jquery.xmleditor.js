...
$.widget( "xml.xmlEditor", {
	...
	uploadXML: function(config) {//HARALAMPIJE!
		/*if (!config || !config.url) { HARALAMPIJE
		...
		/*if (this.textEditor.active) { HARALAMPIJE
			try {
				this.setXMLFromEditor();
			} catch (e) {
		...
		$.ajax({
			...
			success : function(response) {
				...
				alert(response);
				/*var outcome = config.responseHandler(response); HARALMPIJE
...
function MenuBar(editor) {
	...
	this.headerMenuData = [ {
		...
		items : [ {
				...
				binding : "ctrl+alt+s",
				action : function() {
					self.editor.uploadXML.call(self.editor, defaultSubmitConfig);//HARALAMPIJE, tu mozes promeniti u svoj action :)
...
ModifyMenuPanel.prototype.initialize = function (parentContainer) {
	...
	/*if (this.editor.options.enableDocumentStatusPanel) { HARALAMPIJE
		var self = this;
